...
# experiment.                                                                     #
#                                                                                 #
# Here, singe traits are analyzed                                                 #
#                                                                                 #
...
for(n in 1:n.randomizations){
  ...
  for(i in 1:(ncol(functions4.st)-4)){
    ...
    most.common.traits <- c(random.traits-2, 41+random.traits-2) # check!
    ...
    while(P.value < 0.05 && n.predictors.left > 0){
      ...
      if(length(predictors)==0){
        ...
        if(P.value < 0.05){
          ...
          model.formula <- as.formula(
            paste("y", "~", 1, " + ", "(1|year) + (1|block/plot)", sep="")
          )
          best.model <- lmer(model.formula, data=predictors.test)
        }
      }else{
  ...
  R2.model.random[n] <- mean(R2.model,na.rm=T)
